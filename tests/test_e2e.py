...
def test_restore_rebuilds_a_removed_app_from_its_snapshot(harbor_env):
  """`rm` then `restore` puts the app back, compose and routes included.

  `ports-demo` declares no volumes, so this covers the whole path without the
  sudo copy that restoring data volumes needs.
  """
  app_id = "ports-demo"
  assert harbor_env.run("start", app_id).returncode == 0
