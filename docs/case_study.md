...
When we try to run this on a raspberry pi with `harbor up unifi-network-app`, we notice it doesn't seem to be working. Checking the logs, we find that the database uses extensions that our version of arm on the raspberry pi doesn't support. No problem, we can walk back the version of mongo until we find one that works. Between each test, we call `harbor rm --data unifi-network-app` to clear out all data and "start fresh".
