...
- **`harbord` does not refuse to run as root.** `refuse_root` exists in
  `lib/util.py` and the CLI calls it; wiring `daemon/server.py` to it is one
  line, left over from the daemon not existing on the branch that added it.
